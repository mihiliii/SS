...
// Main purpouse of directive %code requires is to include all the necessary
// headers and declarations for the C++ code that will be placed in the
// generated header file before other declarations. It is used for forward
// declarations, #include directives, and other C++ code that should appear
// at the beginning of the generated header file. Commonly used for type
// definitions required by %union and %token declarations.
%code requires {
    #include <vector>
    #include <cstdint>
    struct Operand;
...
void yyerror(const char *s) {
    cout << std::dec << "Parser error at line " << line << ": " << s << endl;
    exit(-1);
}
