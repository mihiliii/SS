...
void Instructions::halt() {
    Assembler::current_section->appendContent(CREATE_INSTRUCTION((uint8_t) OP_CODE::HALT, 0, 0, 0, 0, 0));
}

void Instructions::interrupt() {
    Assembler::current_section->appendContent(CREATE_INSTRUCTION((uint8_t) OP_CODE::INT, 0, 0, 0, 0, 0));
}

void Instructions::iret() {
    Assembler::current_section->appendContent(CREATE_INSTRUCTION(
...
void Instructions::arithmetic_logic_shift(OP_CODE _op, MOD_ALU _mod, uint8_t _gprS, uint8_t _gprD) {
    Assembler::current_section->appendContent(CREATE_INSTRUCTION((uint8_t) _op, (uint8_t) _mod, _gprD, _gprD, _gprS, 0)
    );
}
...
void Instructions::exchange(uint8_t _gprS, uint8_t _gprD) {
    Assembler::current_section->appendContent(CREATE_INSTRUCTION((uint8_t) OP_CODE::XCHG, 0, 0, _gprD, _gprS, 0));
}
