...
std::map<CustomSection*, LiteralTable>& Directives::literal_table_map = Assembler::literal_table_map;

void Directives::sectionDirective(const std::string& _section_name) {
    CustomSectionMap::iterator custom_section_iterator = elf32_file.customSectionMap().find(_section_name);
    ...
    if (custom_section_iterator != elf32_file.customSectionMap().end()) {
        ...
        elf32_file.symbolTable().add(
            current_section->name(), 0, true, current_section->index(), ELF32_ST_INFO(STB_LOCAL, STT_SECTION)
        );
    }
    ...
    for (Operand& node : *_values) {
        ...
        if (node.type == typeid(char*).name()) {
            ...
            if (symbol_entry == nullptr) {
                symbol_entry = &elf32_file.symbolTable().add(symbol_name, 0, false, current_section->index());
            }
...
void Directives::globalDirective(std::vector<Operand>* _symbols) {
    for (Operand& node : *_symbols) {
        ...
        if (symbol_entry == nullptr) {
            elf32_file.symbolTable().add(symbol_name, 0, false, SHN_ABS, ELF32_ST_INFO(STB_GLOBAL, STT_NOTYPE));
        } else {
